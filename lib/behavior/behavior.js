...
Behavior.create({
  ...
  generateSlug(doc) {
    ...
    if (this.options.fieldName && this.options.methodName) {
      throw new Error(
        `Can not generate a slug from a field and method at the same time in the "${Class.getName()}" class`
      );
    }
    ...
    if (this.options.fieldName) {
      ...
      if (!Class.hasField(this.options.fieldName)) {
        throw new Error(
          `Can not create a slug from the "${this.options.fieldName}" field that does not exist in the "${Class.getName()}"class`
        );
      }
      ...
      if (!Class.hasMethod(this.options.methodName)) {
        throw new Error(
          'Can not create a slug from the "' + this.options.methodName +
          '" method that does not exist in the "' + Class.getName() + '" class'
        );
      }
      // Generate a value from a method to make a slug from.
      value = doc[this.options.methodName]();
    }
