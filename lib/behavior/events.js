...
events.beforeSave = function() {
  ...
  var currentSlug = doc.get(behaviorData.slugFieldName);

  // If the current slug is not empty and we can not update, then we have to
  // stop here.
  if (currentSlug && !behaviorData.canUpdate) {
    return;
  }

  // Generate a slug and use a defined separator.
  var slug = generateSlug(value, behaviorData.separator);
  ...
  if (behaviorData.unique) {
    var selector = {};
    selector[behaviorData.slugFieldName] = slug;
    ...
    if (count > 0) {
      ...
      var prefix = slug + behaviorData.separator;
      var re = new RegExp('^' + prefix + '\\d+$');
      selector[behaviorData.slugFieldName] = re;

      // Limit the amount of fields being fetched to only the slug field.
      var fields = {};
      fields[behaviorData.slugFieldName] = 1;
      ...
      Collection.find(selector, fields).forEach(function(d) {
        var dSlug = d.get(behaviorData.slugFieldName);
  ...
  // Set the slug.
  doc.set(behaviorData.slugFieldName, slug);
};
...
events.addBehavior = function(behaviorData) {
  ...
  Class.addField(slugFieldName, {
    type: 'string',
    immutable: !behaviorData.canUpdate,
    default: null
  });
};
